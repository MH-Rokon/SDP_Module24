...
def buy_now(request, book_id):
    ...
    if request.user.account.balance >= book.borrowing_price:
        ...
        messages.success(
            request,
            f'Successfully borrowed the book: {book.title}. Amount {"{:,.2f}".format(float(book.borrowing_price))}$ was withdrawn from your account.'
        )

        send_transaction_email(request.user, book.borrowing_price, "Borrow Book Message", "transaction/borrowbook_email.html")
    else:
        messages.error(
            request,
            f'Insufficient balance to borrow the book: {book.title}.'
        )
